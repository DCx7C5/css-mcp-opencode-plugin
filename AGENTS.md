...
- [done] CONTEXT READING + PLUGIN TESTS — reverse-RPC `session.context.read` push (Python pushes `{id, sessionID}`, JS replies session+messages; fail-safe on client error), `pool.reply()`, `closeBridge()` + `#closed` guards + `EventDebouncer.clear()` for clean test shutdown; `PLUGINS.md` per-plugin reference; README `opencode.json` entries section + PLUGINS.md link; `npm test` (30 tests: inert / E2E vs `client.py --serve` / context-read / lost-brain fail-closed+fail-open / inject / capabilities vs `node:net` brains).
